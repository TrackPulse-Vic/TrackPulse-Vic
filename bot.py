...
from utils.train_art import get_train_art, get_train_fact, get_all_train_types
...
@bot.tree.command(name="train-art", description="Display ASCII art of Victorian trains!")
@app_commands.describe(train_type="Type of train to display (leave empty for random)")
@app_commands.choices(train_type=[
    app_commands.Choice(name="Random", value="random"),
    app_commands.Choice(name="Comeng", value="comeng"),
    app_commands.Choice(name="Siemens Nexas", value="siemens"),
    app_commands.Choice(name="X'Trapolis 100", value="xtrapolis"),
    app_commands.Choice(name="HCMT", value="hcmt"),
    app_commands.Choice(name="VLocity", value="vlocity"),
    app_commands.Choice(name="Sprinter", value="sprinter"),
])
@app_commands.allowed_installs(guilds=True, users=True)
@app_commands.allowed_contexts(guilds=True, dms=True, private_channels=True)
async def train_art(ctx, train_type: str = None):
    await ctx.response.defer()
    log_command(ctx.user.id, 'train-art')
    
    # Get the train art and fact
    if train_type == "random":
        train_type = None
    
    art = get_train_art(train_type)
    fact = get_train_fact(train_type)
    
    # Create embed with ASCII art
    embed = discord.Embed(
        title="🚂 Victorian Train ASCII Art",
        description=f"```{art}```",
        color=discord.Color.blue()
    )
    embed.add_field(name="🎯 Fun Fact", value=fact, inline=False)
    embed.set_footer(text="ASCII art generated by TrackPulse Vic")
